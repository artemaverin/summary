...
Это функции, полученные в результате преобразования другой функции.
...
Это возможность использовать обычный метод в качестве lambda-выражения.
...
В лямбда-выражениях можно использовать final переменные определенные вне выражения.
Переменные внутри lamdba-выражения доступа только внутри блока lamdba-выражения.	
 - ---------------
...
Нужно использовать try-catch блок.
- ---------------
...
Это классы для работы с коллекциями. Созданы, чтобы упростить операции фильтрации, сортировки и другие манипуляции с данными
...
Конвейерные операции преобразуют поток в другой поток. 

Терминальные операции преобразуют поток в результат. Конвейерные операции не выполняются, пока не будет вызвана терминальная операция. Конвейерных операций может быть много, но терминальная только одна.
...
Что такое отложенное исполнение и ленивая загрузка? Чем они отличаются?
Отложенное исполнение - вычисление начинается только в момент запроса его результата.
Ленивая загрузка - работа выполняется только тогда, когда в ней есть необходимость. Например, загрузка ссылочного поля-класса только при явном обращении к этому полю.
- ----------------------
...
Преобразует поток элементов к единичному элементу. Происходит это путем последовательным преобразованием двух элементов.
- ----
...
Можно использовать метод takeWhile. Этот метод частично может реализовать break.
...
Stream.dropWhile - пропускает элементы пока выполняется какоето условие

[к оглавлению](#FP-Lambda-Stream-API)

## 34. Что такое Optional?

Класс-обертка над значением. Позволяет избежать проблем с нулевой ссылкой (NPE)
- ------------------------------
