...
Почему называют “дерево“? Потому что внутри реализован на основе структуры, похожей на дерево: узлы, указатели, ветвления. Их Git использует для того, чтобы понимать, что было изменено, что требует сохранения и т.д.

![image](https://github.com/artemaverin/summary/assets/97846877/2ff259a0-e70e-47b7-bf9c-e2663d9f9adf)

**1) Working Directory (WD) или Working Tree**

Это ваш “рабочий каталог” (проект со всеми файлами). Там находятся все что нужно (включая подпапки).

Когда создаем вручную файлы и папки – они попадают в это дерево.

**2) INDEX (проиндексированные файлы)**, также еще называют Stage (сцена) или Staged area (тоже можно перевести как “сцена”)

Дерево, которое содержит подготовленные изменения для следующего коммита, информацию об отличиях между рабочим каталогом и последним коммитом.

Часто еще называют буфером между Working Directory и HEAD.

Кода выполняем git add – файлы добавляются в INDEX

* Индексация – это процесс добавления данных в Index.

**3) HEAD (история коммитов), Commit history**

Сохраненные версии файлов (коммиты) в ветках. Содержит указатели: на текущую фиксацию (commit) и текущую ветку (branch)

Когда коммитим версии (также вся история логов) – они попадают в это дерево.

Все данные для коммита берутся из INDEX, а не Working Directory!
- ----------------------------------------
...
- ---------------------
Для того, чтобы указать гиту, для каких файлов вы хотите сохранять версии – нужно выполнить команду

git add .
Будут добавлены все файлы из текущей папки. Можно также указывать конкретные файлы (если несколько – то через пробел)

git add first.txt
В этот момент файл переходит из состояния untracked (неотслеживаемый) к новому состоянию tracked (отслеживаемый) – которое позволяет сохранять версии.
...
– git git push origin master - отправить свои локальные изменения на удаленный репозиторий.
...
Обычно ветки создают для того, чтобы попробовать реализовать новый функционал или исправить ошибку – и при этом не затрагивать главную ветку.

В новой ветке вы можете экспериментировать как угодно, не боясь повлиять на основную ветку.

Если вы довели до рабочего состояния доп. ветку – можно выполнить слияние – все изменения попадают из доп. ветки в основную

Самое главное помнить – что новая ветка берет свое начало от текущего коммита (можно сказать, что это общий коммит двух веток)

![image](https://github.com/artemaverin/summary/assets/97846877/e4c850cf-9481-4e58-be58-7b0abb002db0)

Зачем нужно создавать новые ветки?

Одна из основных причин – это то, что вы не будете “захламлять” основную ветку master ненужными коммитами – иначе история будет слишком запутанной.

Вы делаете все нужные коммиты в доп. ветке, а в основную уже попадает конечный результат.

Тем самым master ветка будет более чистой и в ней будут только правильные коммиты (без лишних промежуточных).

Доп. ветку можете потом удалить.
- --------------------
...
Во время работы можно переключаться между ветками.

В этом случае состояние файлов будет сбрасываться на версию последнего коммита ветки. Самое главное помнить в какой ветке вы сейчас находитесь (значения указателей HEAD) – чтобы понимать куда будут добавляться следующие коммиты.

Переключение на ветку: **git checkout name**

Можно создать ветку и сразу переключиться на нее одной командой: **git checkout -b name**

По сути будет выполнено две команды: git branch и git checkout

Чтобы посмотреть, в какой ветке вы сейчас находитесь: **git status**

команда checkout позволяет переключаться между ветками и возвращать состояние файлов как у последнего коммита ветки.

При переключении веток – указатель HEAD попадает на последний коммит ветки (либо получаете Detached HEAD – если не указываете название ветки в параметрах команды).

## 8. Как отменить локальные изменения?

https://job4j.ru/profile/exercise/180/task-view/910

https://javabegin.ru/courses/osnovy-raboty-v-git/lessons/itog-varianty-otkatov/
...
reset – самый опасный вариант

- сбрасывает состояние файлов до нужной версии (параметрами можно регулировать какие деревья именно обновлять: WD, INDEX, HEAD)
- удаляет (отсоединяет от ветки) коммиты из истории (которые стоят выше целевого)
- может поломать общий удаленный репозиторий (с другими участниками), т.к. изменят историю коммитов и можно удалить чужие коммиты
- желательно использовать только в своем локальном репозитории
- перемещает указатель HEAD на целевой коммит

Команда reset:

- используется для возврата к нужному коммиту и удалению всех вышестоящих коммитов
- опасно, если работаете в общем репозитории команды, т.к. можете удалить их коммиты
- обычно выполняется на локальном репозитории, чтобы держать свою историю “чистой”
- с помощью git reflog все еще сможем обращаться к удаленными коммитам

## 10. Расскажите о команде git stash

https://javabegin.ru/courses/osnovy-raboty-v-git/lessons/otkladyvanie-izmenenij-stash/
...
**git stash save "комментарий"**  // сохранить стеш

**git stash list** // список всех стешей

**git stash pop stash@{0}**  // применить изменения и удалить их из набора стешей по ID stash@{0}

**git stash apply stash@{0}** // применить изменения и не удалять данные из набора стешей - если хотите применить их еще раз

**git stash drop stash@{1}**  // удалить отложенные изменения, если вдруг оказались не нужны
