...
**Причины по которым необходимо использовать очередь**
...
_Система перегружена, в этот момент данные события будут лететь в очередь, как только обработчик закончит с задачей, он сразу обратиться в очередь и возьмет следующее событие, благодаря чему система не “ляжет”. Как это бывает, в обычных случаях идет параллельная связь, все запросы обрабатываются сразу, параллельно, из-за того что система не справляется, она может отказать намного быстрее, что и произойдет, при асинхронной коммуникации это минимизировано._
...
Выше был представлен на картинке вариант при котором Сообщение отправляется напрямую от отправителя к получателю
...
![image](https://github.com/artemaverin/summary/assets/97846877/eb02b1ee-6c08-4235-9ccb-2b9c675541a0)
...
https://docs.google.com/document/d/1T43D_f2g-amT_bcawfsc2DbJleNub93WEuR_lSYav6E/edit
...
- планирование
- daily-встречи
- Sprint review - встреча в конце спринта по выполненным задачам (обзор выполненных задач и сложностей с ними, причин невыполнения задач)
- Sprint retrospective - встреча в конце спринта по процессу (обратная связь участников и как можно улучшить организацию работы)
...
## 28. Какие принципы SOLID знаете?
...
Если применять его правильно, он делает код более расширяемым, логичным и читабельным. Когда разработчик создаёт приложение, руководствуясь плохой архитектурой, код получается негибким, даже небольшие изменения в нём могут привести к багам. Поэтому нужно следовать принципам SOLID.
...
- Принцип открытости/закрытости (Open/Closed Principle, OCP) - классы должны быть открыты для расширения, но закрыты для изменения.
- Принцип подстановки Лисков (Liskov Substitution Principle, LSP) - объекты должны заменять друг друга без изменения поведения.
- Принцип интерфейсной абстракции (Interface Segregation Principle, ISP) - модули должны использовать только те интерфейсы, которые им необходимы.
- Принцип инверсии зависимостей (Dependency Inversion Principle, DIP) - зависимости должны быть инвертированы, чтобы модули были независимыми друг от друга.

![image](https://github.com/artemaverin/summary/assets/97846877/6205242c-725f-4c2d-bef3-512bf866a820)
