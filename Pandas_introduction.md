...
При вызове у датафрейма метода **describe()**, получаем информацию по описательной статистики df в разрезе колонок
```python
df = pd.DataFrame({'visits_2021' : [100, 200, 300, 50, 40],
                   'visits_2020' : [90, 100, np.nan, 10, 80],
                   'visits_2019' : [10, np.nan, 20, 16, 80]}, index=['Moscow', 'Kazan', 'Ufa', 'Yakutsk', 'Novosibirsk'])
...
# если нужна информация по строкам, как вариант, можно сначал транспонировать df, и далле применить describe()
print(df.T.describe())
...
```
функцию **describe()** можно также применять у серии

некоторые функции numpy принимают в парамтры датафрейм, например:
```python
import numpy as np
np.abs(df)
```

- функция **unique()** вернет массив уникальных значений серии
- функция **value_counts()** вернет серию со всеми значениями и количество их повторений. При вызове с параметром: ***.value_counts(normalize=True)*** будет посчитано количество элементов в процентах от их общего числа в серии
- функция **isin([])** принимает список значений и возвращает булевую серию True/False если совпало или нет

```python
s2= pd.Series(['a', 'b', 'c', 'a', 'a', 'c'])
print(s2.unique())
['a' 'b' 'c']
